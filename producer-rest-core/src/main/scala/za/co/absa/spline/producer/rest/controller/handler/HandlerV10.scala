...
import za.co.absa.spline.producer.modelmapper.v1_0.ModelMapperV10
import za.co.absa.spline.producer.rest.ProducerAPI.MimeTypeV10
...
trait HandlerV10 {
  ...
  private final val MimeType = MimeTypeV10
  private final val mapper = ModelMapperV10

  @ApiIgnore
  @ResponseStatus(HttpStatus.CREATED)
  @ResponseBody
  @PostMapping(path = Array("/execution-plans"), consumes = Array(MimeType))
  def planHandler(@RequestBody planDTO: v1_0.ExecutionPlan): Future[UUID] = {
    processPlan[v1_0.ExecutionPlan](planDTO, mapper)
  }

  @ApiIgnore
  @ResponseStatus(HttpStatus.CREATED)
  @PostMapping(path = Array("/execution-events"), consumes = Array(MimeType))
  def eventsHandler(@RequestBody eventDTOs: Array[v1_0.ExecutionEvent]): Future[Unit] = {
