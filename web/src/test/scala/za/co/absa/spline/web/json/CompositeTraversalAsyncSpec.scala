...
class CompositeTraversalAsyncSpec extends AsyncFlatSpec with Matchers with MockitoSugar {
  ...
  */
  ...
  def prepareBigLineageMock(readerMock: DataLineageReader): Unit = {
    when(readerMock.loadCompositeByOutput(≡(aUUID))(any())) thenReturn Future.successful(Some(compositeA))
    when(readerMock.loadCompositeByOutput(≡(bUUID))(any())) thenReturn Future.successful(Some(compositeB))
    when(readerMock.loadCompositeByOutput(≡(cUUID))(any())) thenReturn Future.successful(Some(compositeC))
