...
import {VisClusterNode, VisModel, VisNodeIcon} from "../../visjs/vis-model";
...
export class LineageOverviewGraphComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        const lineageContainsDataset = (lin: IDataLineage, dsId: string) => {
        ...
        let reactOnChange = (prevLineage: IDataLineage, nextLineage: IDataLineage, selectedNode: GraphNode) => {
            if (!this.visNetworkService.network || nextLineage.timestamp !== prevLineage.timestamp)
        ...
        this.subscriptions.unshift(combineLatest([lineagePairs$, this.selectedNode$])
            .pipe(
                ...
                distinctUntilChanged(([[, lin0], node0], [[, lin1], node1]) => lin0.timestamp == lin1.timestamp && _.isEqual(node0, node1)))
    ...
    private static buildVisModel(lineage: IDataLineage): VisModel<VisNode, vis.Edge> {
        ...
        let processNodes: VisNode[] = lineage.operations.map((op: IComposite) =>
            new VisProcessNode(
                ...
                getOperationIcon(op).toVisNodeIcon("#337ab7")))
    ...
    private static getTypedSourceIcon(source: ITypedMetaDataSource): VisNodeIcon {
        return getDatasetIcon(source.type)
            .toVisNodeIcon(source.datasetsIds[0].startsWith(ID_PREFIXES.extra) ? "#c0cdd6" : "#337ab7")
    }
