...
import za.co.absa.spline.producer.service.repo.ExecutionPlanDeserFixAspectSpec.{ProceedingJoinPointSpy, sampleExecutionPlanWithRefsAs}

import java.util.UUID

class ExecutionPlanDeserFixAspectSpec
  extends AnyFlatSpec
    with MockitoSugar
    with Matchers {
  ...
  it should "fix ExecutionPlan model in every mathed argument" in {
    ...
    // execute test
    (new ExecutionPlanDeserFixAspect).aroundAdvice(jpSpy)
