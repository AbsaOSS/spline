...
object JDBCWrite {
  def isJDBCWrite(operation: LogicalPlan) = operation match {
    case cmd: SaveIntoDataSourceCommand => cmd.provider == "jdbc"
    case _ => false
  }
}
...
class SaveJdbcCommandParserImpl extends WriteCommandParserNew[LogicalPlan] {
  ...
  def execute(operation: LogicalPlan)(implicit factory: ComponentCreatorFactoryIface): Option[OperationNodeBuilder] = {
    operation match {
      case op: SaveIntoDataSourceCommand if JDBCWrite.isJDBCWrite(operation) => {
        Some(new OperationNodeBuilder(factory, op)  {
      ...
      case op: SaveIntoDataSourceCommand if ! JDBCWrite.isJDBCWrite(operation) => {
        Some(new OperationNodeBuilder(factory, op)  {
          ...
          override def build(): WriteOperation = {
            ...
            WriteOperation(
              outputSource = "file:" + op.options.getOrElse("path", ""),
              append = op.mode == SaveMode.Append,
              id = id,
              childIds = childIds(),
              schema = makeSchema(),
