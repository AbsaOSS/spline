...
class FSReadNodeBuilderSpec extends FunSpec with MockitoSugar with Matchers with SparkFixture {
  ...
  describe("support for different types of data source") {
    ...
    it("should support JDBC") {
      withNewSparkSession(spark => {
        ...
        val bldr = new FSReadNodeBuilder(df.queryExecution.analyzed.asInstanceOf[LogicalRelation]) with FSUnawareBuilder
        val readOp = bldr.build()

        readOp.params should contain
        "sourceType" -> "JDBC"
    ...
    it("should handle unrecognized source type") {
      val bldr = new FSReadNodeBuilder(LogicalRelation(FooBarRelation)) with FSUnawareBuilder

      val readOp = bldr.build()

      readOp.params("sourceType") shouldEqual "???: za.co.absa.spline.harvester.FooBarRelation$"
