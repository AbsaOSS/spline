...
class AttributeDependencySolver_ImpactSpec extends AnyFlatSpec with Matchers {

  import za.co.absa.spline.consumer.service.internal.AttributeDependencySolver_ImpactSpec._
  ...
  it should "resolve basic select with sum" in {
    ...
    assertImpactEquals(operations, A, Seq(C -> A), Seq((A, 0, Nil), (C, 1, Seq(2))))
    assertImpactEquals(operations, B, Seq(C -> B), Seq((B, 0, Nil), (C, 1, Seq(2))))
    assertImpactEquals(operations, C, Seq.empty, Seq((C, 1, Seq(2))))
  }
  ...
  it should "resolve three dependent operation" in {
    ...
    assertImpactEquals(operations, A, Seq(D -> C, C -> B, B -> A), Seq((A, 0, Nil), (B, 1, Nil), (C, 2, Nil), (D, 3, Seq(4))))
    assertImpactEquals(operations, B, Seq(D -> C, C -> B), Seq((B, 1, Nil), (C, 2, Nil), (D, 3, Seq(4))))
    assertImpactEquals(operations, C, Seq(D -> C), Seq((C, 2, Nil), (D, 3, Seq(4))))
    assertImpactEquals(operations, D, Seq.empty, Seq((D, 3, Seq(4))))
  }
  ...
  it should "resolve chain of several operations including an expression" in {
    ...
    assertImpactEquals(operations, C,
      Seq(F -> E, E -> C),
      Seq((C, 1, Nil), (E, 2, Nil), (F, 3, Seq(4))))
    assertImpactEquals(operations, D,
      Seq(F -> E, E -> D),
      Seq((D, 1, Nil), (E, 2, Nil), (F, 3, Seq(4))))
    assertImpactEquals(operations, E,
      Seq(F -> E),
      Seq((E, 2, Nil), (F, 3, Seq(4))))
    assertImpactEquals(operations, F,
      Seq.empty,
      Seq((F, 3, Seq(4))))
  }
  ...
  it should "resolve aggregation" in {
    ...
    assertImpactEquals(operations, A, Seq(C -> A), Seq((A, 0, Nil), (C, 1, Seq(2))))
    assertImpactEquals(operations, B, Seq(D -> B), Seq((B, 0, Nil), (D, 1, Seq(2))))
    assertImpactEquals(operations, C, Nil, Seq((C, 1, Seq(2))))
    assertImpactEquals(operations, D, Nil, Seq((D, 1, Seq(2))))
  }
  ...
  it should "resolve generation" in {
    ...
    assertImpactEquals(operations, A, Seq(B -> A), Seq((A, 0, Nil), (B, 1, Seq(2))))
    assertImpactEquals(operations, B, Nil, Seq((B, 1, Seq(2))))
  }
  ...
  it should "resolve io operation correctly" in {
    ...
    assertImpactEquals(operations, A, Seq(D -> A), Seq((A, 1, Nil), (D, 2, Seq(3))))
    assertImpactEquals(operations, B, Nil, Seq((B, 1, Nil)))
    assertImpactEquals(operations, C, Seq(D -> C), Seq((C, 1, Nil), (D, 2, Seq(3))))
    assertImpactEquals(operations, D, Seq.empty, Seq((D, 2, Seq(3))))
  }
  ...
  it should "resolve filter operation correctly" in {
    ...
    assertImpactEquals(operations, A, Seq(B -> A), Seq((A, 0, Seq(1)), (B, 2, Seq(3, 4, 5))))
    assertImpactEquals(operations, B, Seq.empty, Seq((B, 2, Seq(3, 4, 5))))
  }
  ...
  it should "resolve operation with several sources correctly" in {
    ...
    assertImpactEquals(operations, A,
      Seq(F -> D, D -> A),
      Seq((A, 0, Nil), (D, 3, Seq(5)), (F, 6, Seq(8, 9))))
    assertImpactEquals(operations, B,
      Seq(F -> E, E -> B),
      Seq((B, 1, Nil), (E, 4, Seq(5)), (F, 6, Seq(8, 9))))
    assertImpactEquals(operations, C,
      Seq(G -> C),
      Seq((C, 2, Nil), (G, 7, Seq(8, 9))))
    assertImpactEquals(operations, D,
      Seq(F -> D),
      Seq((D, 3, Seq(5)), (F, 6, Seq(8, 9))))
    assertImpactEquals(operations, E,
      Seq(F -> E),
      Seq((E, 4, Seq(5)), (F, 6, Seq(8, 9))))
    assertImpactEquals(operations, F,
      Nil,
      Seq((F, 6, Seq(8, 9))))
    assertImpactEquals(operations, G,
      Nil,
      Seq((G, 7, Seq(8, 9))))
  }
