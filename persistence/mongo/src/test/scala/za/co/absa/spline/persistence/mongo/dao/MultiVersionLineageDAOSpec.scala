...
class MultiVersionLineageDAOSpec extends AsyncFlatSpec with MockitoSugar with Matchers with OneInstancePerTest {
  ...
  private val dao = {
    when(daoV1Mock.findDatasetDescriptors(any(), any())(any())).thenReturn(Future.successful(CloseableIterable.empty[DBObject]))
    when(daoV2Mock.findDatasetDescriptors(any(), any())(any())).thenReturn(Future.successful(CloseableIterable.empty[DBObject]))
    when(daoV3Mock.findDatasetDescriptors(any(), any())(any())).thenReturn(Future.successful(CloseableIterable.empty[DBObject]))
    when(daoV4Mock.findDatasetDescriptors(any(), any())(any())).thenReturn(Future.successful(CloseableIterable.empty[DBObject]))
    ...
    new MultiVersionLineageDAO(daoV1Mock, daoV2Mock, daoV3Mock, daoV4Mock)
  }

  it should "find all items" in {

    dao.findDatasetDescriptors(None, EntireLatestContent).map(_ => {
      verify(daoV1Mock, never).findDatasetDescriptors(any(), any())(any())
      ...
      verify(daoV4Mock, never).findDatasetDescriptors(any(), any())(any())
      Assertions.succeed
    })
  }
  ...
  it should "find first page inside a shard" in {
    dao.findDatasetDescriptors(None, EntireLatestContent.copy(offset = 0, size = 7)).map(_ => {
      ...
      verify(daoV3Mock, never).findDatasetDescriptors(any(), any())(any())
      verify(daoV4Mock, never).findDatasetDescriptors(any(), any())(any())
      Assertions.succeed
    })
  }
  ...
  it should "find second page inside a shard" in {
    dao.findDatasetDescriptors(None, EntireLatestContent.copy(offset = 15, size = 5)).map(_ => {
      verify(daoV1Mock, never).findDatasetDescriptors(any(), any())(any())
      verify(daoV2Mock, never).findDatasetDescriptors(any(), any())(any())
