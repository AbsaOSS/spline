...
object MutableLineageUpgraderV4Spec {
  ...
  trait VersionedLineageDAODummy extends VersionedLineageDAO {
    ...
    override def upgrader: Option[VersionUpgrader] = noImpl

    override def save(lineage: DBObject)(implicit e: ExecutionContext): Future[Unit] = noImpl

    override def saveProgress(progressDBObject: ProgressDBObject)(implicit e: ExecutionContext): Future[Unit] = noImpl
    ...
    override def findDatasetDescriptors(maybeText: Option[String], pageRequest: PageRequest)
                                       (implicit ec: ExecutionContext): Future[CloseableIterable[DescriptorDBObject]] = noImpl

    override def findDatasetDescriptors(maybeText: Option[String], intervalRequest: IntervalPageRequest)
                                       (implicit ec: ExecutionContext): Future[CloseableIterable[DescriptorDBObject]] = noImpl

    override def getDatasetDescriptor(id: UUID)(implicit ec: ExecutionContext): Future[Option[DescriptorDBObject]] = noImpl
    ...
    override def getLineagesByPathAndInterval(path: String, start: Long, end: Long)
                                             (implicit ex: ExecutionContext): Future[CloseableIterable[DBObject]] = noImpl
  }
