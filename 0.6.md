...
```shell
docker run -it --rm absaoss/spline-admin:{{page.core_version}} db-init arangodb://localhost/spline
```
...
**Note**: There is no automatic rollback for database migration! It migration fails for any reason you should start over from the clean database snapshot. So don't forget to make backups before running the migration tool!
