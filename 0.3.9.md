...
### download [Spline Web UI executable JAR](https://search.maven.org/remotecontent?filepath=za/co/absa/spline/spline-web/{{site.version}}/spline-web-{{site.version}}-exec-war.jar) and run:
```shell
java -jar spline-web-{{site.version}}-exec-war.jar -Dspline.mongodb.url=...
```
...
# Execute pyspark with Fat-JAR on class path:
  pyspark --jars 'target/spline-sample-{{site.version}}-SNAPSHOT.jar'
