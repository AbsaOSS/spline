...
object LineageDispatcher {
  ...
  def apply(configuration: Configuration): LineageDispatcher = {
    ...
    new LineageDispatcher(publishTimeoutSeconds, publishUrl, HttpSenderImpl)
  }
}

class LineageDispatcher(publishTimeoutSeconds: Long, publishUrl: String, httpSender: HttpSender) extends Logging {
